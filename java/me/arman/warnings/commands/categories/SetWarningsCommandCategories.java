...
public class SetWarningsCommandCategories implements CommandExecutor {

	static Main plugin;

	public SetWarningsCommandCategories(Main instance) {
		plugin = instance;
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		if (cmd.getName().equalsIgnoreCase("setwarnings")) {
			...
			if (args.length <= 2) {
				sender.sendMessage(ChatColor.RED + "Usage: /setwarnings <player> <amount> <category>");
				return true;
				...
				plugin.dFile.getConfig().set(target.getUniqueId().toString() + ".warnings." + args[2], num);
				if (plugin.getConfig().getBoolean("logSpecificInfo")) {
					...
					l.add("Warnings set to " + "(" + num + ")" + " by " + "(" + sender.getName() + ")" + " in category "
